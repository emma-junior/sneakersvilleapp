...
const PaymentForm = () => {
  ...
  const formFilled = Object.values(data && data.address).every(value => value);
    ...
    useEffect(() => {
      const makeRequest = async () => {
        try {
          ...
          navigate("/success", { state: { stripeData: data,
          products: Cart, } })
        ...
        <div className='form-wrapper'>
            ...
            <div>
              <div className='address-input'>
                  <input placeholder='city' type="text" value={data.address.city} onChange={(e) => setData({...data, address: {...data.address, city: e.target.value} })} />
                  <input placeholder='state' type="text" value={data.address.state} onChange={(e) => setData({...data,address: {...data.address, state: e.target.value}})} />
              </div>
            </div>
            <div className='btn'>
              {formFilled ? <button className='submit-btn' onClick={payWithPaystack}>Submit</button>: <button className='inactive-btn'>Submit</button>}
            </div>
        </div>
    </section>
  )
